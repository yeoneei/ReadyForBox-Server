...
router.get('/', async (req, res) => {
    try {
        ...
        if (!main_category_id || !flag) {
            ...
            if(flag == 2) {
                 let query = "SELECT main_img,name,price,ROUND((1-(sale_ratio*0.01))*price,0) AS saled_price FROM products WHERE main_category_id=? AND is_package = 1 ORDER BY created_at DESC";
            ...
            // 가격 낮은 순
                ...
                let query1 = "SELECT main_img,name,price,ROUND((1-(sale_ratio*0.01))*price,0) AS saled_price FROM products WHERE main_category_id=? AND is_package = 1 ORDER BY saled_price ASC";
                ...
                let query2 = "SELECT main_img,name,price,ROUND((1-(sale_ratio*0.01))*price,0) AS saled_price FROM products WHERE main_category_id=? AND is_package = 1 ORDER BY saled_price DESC";
