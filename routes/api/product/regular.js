...
router.get('/', async (req, res) => {
    try {
        ...
        if (!main_category_id || !flag) {
            ...
            if (flag == 3) {
                let query1 = "SELECT main_img,name,content,price,(1-(sale_ratio*0.01))*price AS saled_price FROM products WHERE main_category_id=? AND is_package = 0 ORDER BY saled_price DESC";
                ...
                let query2 = "SELECT main_img,name,content,price,(1-(sale_ratio*0.01))*price AS saled_price FROM products WHERE main_category_id=? AND is_package = 0 ORDER BY saled_price ASC";
